...
\paragraph{Mesures wall-clock à $n = 1\,000$ et $n = 10\,000$.}

Le passage à des tailles plus réalistes confirme l'inversion attendue.
Mesures prises en ligne de commande avec \texttt{time rars
test\_tri\_XXX.asm > /dev/null 2>\&1} --- stdout redirigé pour neutraliser
le coût de rendu terminal des \texttt{print\_tab}, seuls restent les
\texttt{ecall} système et le tri proprement dit.

\begin{table}[h]
\centering
\begin{tabular}{lrrrr}
\toprule
\textbf{Algorithme} & \textbf{$n=20$ (instr.)} & \textbf{$n=1\,000$} & \textbf{$n=10\,000$} & \textbf{Complexité} \\
\midrule
Tri à bulles      & 3\,034 & 2,50 s & 231,6 s & $O(n^2)$ \\
Tri par sélection & ---    & 2,57 s & 223,9 s & $O(n^2)$ \\
Tri par tas       & 3\,375 & \textbf{0,41 s} & \textbf{2,51 s} & $O(n \log n)$ \\
\bottomrule
\end{tabular}
\caption{Temps wall-clock pour chaque algorithme aux trois tailles
de test. Les colonnes $n=1\,000$ et $n=10\,000$ sont en secondes ;
la baseline JVM (programme vide) est d'environ $187$ ms.}
\end{table}

\paragraph{Observations clés.}

\begin{itemize}
    \item À \textbf{$n=20$}, les constantes dominent et le tri par tas
          est même \emph{plus coûteux} que la bulle en nombre
          d'instructions ($3\,375$ vs $3\,034$).
    \item À \textbf{$n=1\,000$}, l'écart O$(n^2)$ vs O$(n \log n)$ devient
          visible : tas est $\times 6$ plus rapide que bulle et sélection.
    \item À \textbf{$n=10\,000$}, l'écart explose : tas est $\times 92$
          plus rapide. Ratio théorique $n / \log_2 n \approx 750$ ;
          la différence s'explique par le coût des \texttt{ecall} de
          \texttt{print\_tab} qui persiste même avec stdout redirigé,
          plus significatif pour les algos rapides.
    \item \textbf{Bulle et sélection sont empiriquement indiscernables}
          en wall-clock à $n=1\,000$ et $n=10\,000$, malgré un nombre
          d'échanges radicalement différent ($\sim n^2/2$ pour bulle,
          exactement $n-1$ pour sélection). À cette échelle, le coût
          des \textbf{comparaisons} (identique pour les deux) domine
          celui des échanges.
\end{itemize}

\paragraph{Baseline et coût de \texttt{print\_tab}.}

Le démarrage \guillemotleft{} à vide \guillemotright{} de RARS (un
\texttt{.asm} qui termine immédiatement) $=$ \textbf{187 ms}. Tout ce
qui dépasse cette baseline est imputable au programme de test.

Les \texttt{print\_tab} contribuent à hauteur du nombre d'\texttt{ecall}
émis, même lorsque stdout est redirigé vers \texttt{/dev/null} : chaque
\texttt{ecall} passe par la machine virtuelle RARS, qui a un coût fixe
non négligeable. À $n = 10\,000$, cela représente $4 \cdot 10^4$
\texttt{ecall} (deux appels à \texttt{print\_tab}, chacun faisant
$n$ fois \texttt{print\_int} $+$ \texttt{print\_string}), dont le coût
devient dominant pour le tri par tas.

\paragraph{Conclusion : trois points de mesure complémentaires.}

\begin{itemize}
    \item \textbf{$n = 20$ (instructions)} : les constantes dominent ;
          la complexité asymptotique ne suffit pas à prédire l'ordre.
          Utile pour illustrer le \emph{piège du petit $n$}.
    \item \textbf{$n = 1\,000$ (wall-clock)} : l'écart O$(n^2)$ vs
          O$(n \log n)$ devient visible sans attendre ; taille de dev
          confortable pour itérer.
    \item \textbf{$n = 10\,000$ (wall-clock)} : mise en évidence finale
          de la bascule ; valide expérimentalement la complexité
          annoncée.
\end{itemize}
